...
@section('content')
    ...
    $isNew = !isset($log_id) || !$log_id;
            ...
            <div class="flex flex-col gap-5">
                ...
                <form
                    x-data="{ log_id: @js($log_id), heroes: @js($initialHeroes), about: @js($initialAbout), service: @js($initialService), showModal: false, approveModal: false, rejectModal: false, isSubmitting: false, isDraft: false, notes: '', uploadWarning: false }"
                    @submit="isSubmitting = true" action="{{ $isNew ? route('cms.store') : route('cms.update', $log_id) }}"
                                ...
                                <div class="shadow-md border border-gray-300 rounded-lg p-4 flex flex-col">
                                    ...
                                    <div class="mt-4 flex flex-col gap-3">
                                        ...
                                        <div x-show="hero.is_masked == true" class="flex flex-col gap-1">
                                            ...
                                            <input type="number" x-model="hero.opacity" :name="`heroes[${index}][opacity]`" min="0"
                                                max="100" :required="hero.is_masked == true"
                                                class="p-2 text-sm text-gray-500 font-medium border border-gray-300 rounded-md focus:outline-none focus:border-blue-400">
                                        </div>
                                        ...
                                        <div x-data="{ 
                                                                                                                                                        ...
                                                                                                                                                        fileSize: hero.image?.image_size ? (hero.image.image_size / 1024).toFixed(1) + ' KB' : null
                                                                                                                                                    }"
                                        ...
                                        <div x-data="{
                                                                                                                                                    ...
                                                                                                                                                    mobileFileSize: hero.image_mobile?.image_size ? (hero.image.image_size / 1024).toFixed(1) + ' KB' : null
                                                                                                                                                }"
                                            class="flex flex-col gap-1">
                    ...
                    <div class="shadow-md border border-gray-300 p-5 rounded-lg flex flex-col mt-5">
                        ...
                        <div x-data="{
                                                                                                                    ...
                                                                                                                    aboutFileSize: about.image?.image_size ? (about.image.image_size / 1024).toFixed(2) + ' KB' : null
                                                                                                                }" class="mt-4">
                            ...
                            <template x-for="(color, index) in service.bg_config.colors" :key="index">
                                <div class="flex flex-row items-center gap-2">
                                    ...
                                    <button x-show="service.bg_config.colors.length > 1" type="button"
                                        class="px-4 py-2 bg-red-500 text-white rounded-lg font-semibold text-sm"
                                        @click="
                            ...
                            <template x-for="(detail, index) in service.details" :key="index">
                                <div class="p-4 shadow-md border border-gray-300 rounded-lg flex flex-col mb-2">
                                    ...
                                    <div x-data="{
                                                                                                                                ...
                                                                                                                                serviceFileSize: detail.image?.image_size ? (detail.image.image_size / 1024).toFixed(1) + ' KB' : null
                                                                                                                            }"
                                        ...
                                        <div x-show="!serviceImgUrl"
                                            ...
                                            <input type="file" :name="`service[details][${index}][image]`" accept="image/*"
                                                class="absolute z-20 inset-0 opacity-0"
                                                @change="
                                                                                                                                        const file = event.target.files[0];
                            ...
                            @if ($isNew)
                                <div class="flex flex-row items-center gap-3">
                                    <button type="submit" name="status" value="draft"
                                        class="px-4 py-2 rounded-lg bg-gray-500 hover:bg-gray-700">
                                        <p class="text-white text-sm font-bold">Simpan Draft</p>
                                    </button>
                                    <button type="button" @click="showModal = true"
                                        class="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-700">
                                        <p class="text-white text-sm font-bold">Simpan</p>
                                    </button>
                                </div>
                            @elseif (isset($log_id) && $log_id && !$isNew)
                                @if ($status === 'draft')
                                    <button type="submit" name="status" value="draft"
                                        class="px-4 py-2 rounded-lg bg-gray-500 hover:bg-gray-700">
                                        <p class="text-white text-sm font-bold">Simpan Draft</p>
                                ...
                                @endif
                                <button type="button" @click="showModal = true"
                                    class="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-700 ml-3">
                                    <p class="text-white text-sm font-bold">Simpan</p>
                                </button>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-5">
                            <div class="flex flex-row items-center gap-3">
                                <button type="button" @click="rejectModal = true"
                                    ...
                                    <p class="text-white font-semibold text-sm">Tolak</p>
                                </button>
                                <button type="button" @click="approveModal = true"
                                    class="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-700">
                                    <p class="text-white font-semibold text-sm">Setujui</p>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="min-w-md bg-white rounded-lg shadow-md flex flex-col p-4">
                            ...
                            <div class="flex flex-row items-center justify-center gap-3 mt-2">
                                <button type="button" @click="showModal = false"
                                    class="px-4 py-2 bg-red-500 text-white font-semibold text-sm rounded-lg hover:bg-red-700 cursor-pointer">Batal</button>
                                <button type="submit" name="status" value="waiting_approval"
                                    class="px-4 py-2 rounded-lg bg-gray-700 text-sm font-semibold text-white hover:bg-gray-900 cursor-pointer">Simpan</button>
                            </div>
                        </div>
                    </div>

                    {{-- modal save and update end --}}

                    {{-- modal approve and reject start --}}

                    {{-- approve modal --}}
                    <div x-show="approveModal" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                        <div class="min-w-md flex flex-col bg-white rounded-lg shadow-md p-4">
                            <p class="text-sm font-semibold text-center">Apakah anda yakin ingin menyetujui konten ini untuk
                                tayang?</p>
                            <p class="text-xs font-medium mt-2 text-center">*konten yang disetujui akan tampil di website <span><a
                                        class="text-blue-500 hover:underline" href="http://localhost:5173" target="_blank">BPI
                                        Uniform</a></span></p>
                            <div class="flex flex-row justify-center items-center gap-3 mt-4">
                                <button type="button" @click="approveModal = false" class="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-700 cursor-pointer" type="submit" name="status" value="rejected">
                                    <p class="text-xs font-semibold text-white">Tidak</p>
                                </button>
                                <button type="submit" class="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-700 cursor-pointer" type="submit" name="status" value="approved">
                                    <p class="text-xs font-semibold text-white">Setujui</p>
                                </button>
                            </div>
                        </div>
                    </div>

                    <div x-show="rejectModal" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                        <div class="min-w-md flex flex-col bg-white rounded-lg shadow-md p-4">
                            <p class="text-sm font-semibold">Berikan alasan penolakan konten</p>
                            <textarea class="mt-2 p-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-gray-500"
                                    placeholder="Catatan perubahan" x-model="notes" name="notes"></textarea>

                            <div class="flex flex-row items-center justify-center gap-3 mt-4">
                                <button type="button" @click="rejectModal = false" class="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-700 cursor-pointer">
                                    <p class="text-xs font-semibold text-white">Batal</p>
                                </button>
                                <button type="submit" name="status" value="rejected" class="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-700 cursor-pointer">
                                    <p class="text-xs font-semibold text-white">Lanjut</p>
                                </button>
                            </div>
                        </div>
                    </div>

                    {{-- modal approve and reject end --}}
